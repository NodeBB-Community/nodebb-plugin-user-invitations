...
exports.loginCheck = ({uid, req}) => {
  const ref = req.cookies.pluginUserInvitationsRef

  if (!ref) return

  log(`User ${uid} was referred.`)

  async.waterfall([
    async.apply(backend.referral, {action: 'isMember', value: uid}),
    (isMember, next) => {
      if (isMember) return next(new Error('Member has already been referred'))
    ...
    (email, next) => {
      User.getUidByUsername(ref, (err, uid) => next(err, { referral: email, referring: uid }))
    },
    (data, next) => {
    ...
    (data, next) => {
      ...
      db.sortedSetAdd('invitation:uid', data.referring, data.referral, err => next(err, data))
    },
  ], (error, data) => {
    if (error) return log(error)
    acceptUserInvitation(uid, data.referral, data.referring)
  })
}
